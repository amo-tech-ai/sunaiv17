...
import { Tag, Clock, ArrowRightCircle } from 'lucide-react';
...
export const TaskBoard: React.FC<TaskBoardProps> = ({ tasks, onUpdateTaskStatus }) => {
        ...
        return (
          <div key={col.id} className="flex flex-col bg-sun-right/50 border border-sun-border rounded-sm h-full">
            ...
            <div className="p-4 space-y-3 overflow-y-auto flex-1 no-scrollbar">
              {colTasks.length > 0 ? (
                colTasks.map(task => (
                  <div key={task.id} className="bg-white border border-sun-border p-4 rounded-sm shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
